...
export default function ProfileModal({ isOpen, onClose }) {
  ...
  async function saveProfile(e) {
    ...
    try {
      ...
      setNotice({ type: 'info', text: 'Profile saved' });
      onClose?.();
    } catch (e) {
      setNotice({ type: 'error', text: e.response?.data || 'Failed to save profile' });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function uploadResume(e) {
    ...
    try {
      ...
      setNotice({ type: 'info', text: 'Resume uploaded' });
      ...
      setNotice({ type: 'error', text: e.response?.data || 'Failed to upload resume' });
    } finally {
      setUploading(false);
    }
  }
  ...
  async function uploadLogo(e) {
    ...
    try {
      ...
      setNotice({ type: 'info', text: 'Company logo uploaded' });
      const res = await api.get("/me");
      const user = res.data;
      setForm({
        ...form,
      ...
      setNotice({ type: 'error', text: e.response?.data || 'Failed to upload company logo' });
    } finally {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Your Profile" size="lg">
      {notice && (
        <div style={{
          ...
          padding: '10px 12px',
          borderRadius: 6,
          background: notice.type === 'error' ? '#fee2e2' : notice.type === 'warning' ? '#fef3c7' : '#e0f2fe',
          color: '#111827',
          display: 'flex',
        ...
        <div className="card" style={{ marginTop: 16, padding: 16 }}>
          ...
          <div>
            <button
              ...
              onClick={async () => {
                if (!pwd.currentPassword || !pwd.newPassword) { setNotice({ type: 'warning', text: 'Please fill all password fields' }); return; }
                if (pwd.newPassword !== pwd.confirm) { setNotice({ type: 'warning', text: 'Passwords do not match' }); return; }
                setChangingPwd(true);
                try {
                  await api.post('/auth/change-password', { currentPassword: pwd.currentPassword, newPassword: pwd.newPassword });
                  setNotice({ type: 'info', text: 'Password changed' });
                  setPwd({ currentPassword: '', newPassword: '', confirm: '' });
                } catch (e) {
                  setNotice({ type: 'error', text: e.response?.data || 'Failed to change password' });
                } finally {
