  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setNotice({ type: 'error', text: 'Failed to load job' });
      } finally {
        setLoading(false);
      }
    })();
  ...
  async function submit(e) {
    ...
    try {
      ...
      try {
        ...
        if (!hasApplied) {
          if (resumeFile) {
            ...
            if (initStatus !== 'READY_FOR_TEST') {
              const msg = `Application status: ${initStatus || 'unknown'}. ${suggestion ? ' Details: ' + suggestion : ''}`;
              setNotice({ type: 'warning', text: msg });
              return;
            }
          } else {
            setNotice({ type: 'info', text: 'Please submit your resume first (Profile -> Upload Resume or use Resume Analysis) before applying.' });
            return;
          }
        }
        ...
        setNotice({ type: 'error', text: postErr.response?.data || postErr.message || 'Failed to submit application' });
      }
