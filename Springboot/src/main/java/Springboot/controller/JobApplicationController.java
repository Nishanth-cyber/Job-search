...
public class JobApplicationController {
    ...
    public ResponseEntity<?> checkApplication(@PathVariable String jobId) {
        String jobSeekerId = getCurrentUserId();
        boolean hasApplied = jobApplicationService.hasUserAppliedForJob(jobId, jobSeekerId);
        return ResponseEntity.ok(Map.of("hasApplied", hasApplied));
    }

    // Submit resume for N8N analysis (initial application)
    @PostMapping(value = "/initial", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> submitInitialApplication(
        ...
        try {
            JobApplicationModel application = jobApplicationService.createInitialApplication(
                jobId, jobSeekerId, resume, coverLetter);
    ...
    // Preview N8N analysis (no persistence)
    @PostMapping(value = "/preview", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> previewAnalysis(
            @RequestParam String jobId,
            @RequestPart MultipartFile resume
    ) {
        String jobSeekerId = getCurrentUserId();
        try {
            Map<String, Object> resp = jobApplicationService.previewAnalysis(jobId, jobSeekerId, resume);
    ...
    @GetMapping
    public ResponseEntity<?> getApplicationsByQuery(@RequestParam(required = false) String jobId) {
        if (jobId == null || jobId.isBlank()) {
            return ResponseEntity.badRequest().body("Missing jobId");
        }
        String requesterId = getCurrentUserId();
        Optional<JobModel> job = jobService.getJobById(jobId);
        if (job.isPresent() && job.get().getPostedBy().equals(requesterId)) {
            return ResponseEntity.ok(jobApplicationService.getApplicationsForJob(jobId));
        }
        List<JobApplicationModel> own = jobApplicationService
                .getApplicationsByJobSeeker(requesterId)
                .stream()
